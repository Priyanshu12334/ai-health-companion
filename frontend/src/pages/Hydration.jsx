...
const Hydration = () => {
 ...
 <div className="space-y-6 max-w-2xl mx-auto w-full">
  <div className="flex justify-between items-center">
    <h2 className="text-2xl font-bold flex items-center gap-2"><Droplets className="text-sky-600"/> Hydration Tracking</h2>
 ...
 <div className="absolute top-0 left-0 w-full h-full bg-sky-500/10 pointer-events-none" style={{ height: `${percent}%`, top: 'auto', bottom: 0, transition: 'height 1s ease-in-out' }}></div>
 <div className="relative z-10">
 <p className="text-sm text-text-secondary mb-2">Daily Goal: {data.goal}ml</p>
 <h1 className="text-6xl font-extrabold text-sky-600 dark:text-sky-600 mb-4">{data.total}<span className="text-2xl text-text-secondary font-medium ml-1">ml</span></h1>
 ...
 <button onClick={() => addWater(250)} disabled={adding} className="glass-card p-4 flex flex-col items-center hover:bg-sky-100 dark:hover:bg-sky-900/20 transition-colors border-sky-200 dark:border-sky-800">
 ...
 <button onClick={() => addWater(500)} disabled={adding} className="glass-card p-4 flex flex-col items-center hover:bg-sky-100 dark:hover:bg-sky-900/20 transition-colors border-sky-200 dark:border-sky-800">
 <Plus className="w-6 h-6 text-sky-500 mb-2" />
 <span className="font-bold">500ml</span>
 <span className="text-xs text-text-secondary">Bottle</span>
 </button>
 <button onClick={() => addWater(1000)} disabled={adding} className="glass-card p-4 flex flex-col items-center hover:bg-sky-100 dark:hover:bg-sky-900/20 transition-colors border-sky-200 dark:border-sky-800">
 <Plus className="w-6 h-6 text-sky-500 mb-2" />
